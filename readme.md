...
### 例) レプリケーション有効化のサンプルコマンド

```powershell
.\Enable-WorkspaceReplication.ps1 `
  -SubscriptionId "11111111-2222-3333-4444-555555555555" `
  -ResourceGroupName "rg-sentinel-jpe" `
  -WorkspaceName "law-sentinel-jpe-prod" `
  -PrimaryRegion "japaneast" `
  -SecondaryRegion "japanwest"
```

<img src="./images/1-レプリケーション有効化.png" alt="レプリケーション有効化" width="900" />

*レプリケーション有効化実行後の画面例*

### 例) フェールオーバー後のイメージ
フェールオーバーすると、セカンダリリージョンがプライマリに切り替わります。新たなリソースとして見えるのではなく、ポータル上は既存の同じリソースを参照し続けますが、リージョンが切り替わります。

<img src="./images/3-フェールオーバー2.png" alt="フェールオーバー後のイメージ" width="900" />

*フェールオーバー後のワークスペースの状態例（セカンダリリージョンがプライマリに切り替わる）*
